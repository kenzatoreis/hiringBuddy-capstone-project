# hbapp/models_docs.py
from sqlalchemy import (
    Column, Integer, String, DateTime, ForeignKey, Text, JSON, CheckConstraint
)
from sqlalchemy.sql import func
from sqlalchemy.orm import relationship
from db import Base  # you already have this
...
class Document(Base):
    ...
    __table_args__ = (
        CheckConstraint("type in ('cv','jd')", name="documents_type_chk"),
    )
...
class DocStructured(Base):
    ...
    document_id = Column(Integer, ForeignKey("documents.id", ondelete="CASCADE"), primary_key=True)
    fields = Column(JSON)  # e.g. {"skills":[...], "education":[...]}
